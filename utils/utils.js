...
async function createEscrowContractId(bytecodeFileId,gasLimit,client,providerId,moId,purchaserId) {
    console.log("################### Calling createContractId function #######################################");
    // Instantiate the contract instance
    const contractTx = await new ContractCreateTransaction()
        //Set the file ID of the Hedera file storing the bytecode
        ...
        //Set the gas to instantiate the contract
        .setGas(gasLimit)
        .setConstructorParameters(new ContractFunctionParameters().addAddress(providerId.toSolidityAddress())
        .addAddress(moId.toSolidityAddress())
        .addAddress(purchaserId.toSolidityAddress()));
    //Submit the transaction to the Hedera test network
    const contractResponse = await contractTx.execute(client);
