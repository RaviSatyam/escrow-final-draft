...
export class UsersController {
  ...
  // Api to retrieve all milestone data from mirror node corresponding to user and project id
  @Get('ms_data/:user_id/:project_id')
  async getMsDataFromMirrorNode(@Param('user_id') user_id: number, @Param('project_id') project_id: number): Promise<string> {
    const userID = Number(user_id);
    const projectID = Number(project_id);
    const milestones = this.usersService.getMsDataFromMirrorNode(userID, projectID);
